...
for (i, p) in enumerate(papers)
    ...
    println("[*" * p["venue"] * "*](" * p["url"] * ")" * (haskey(p, "venue-details") ? ", " * p["venue-details"] : ""))
    println("")
end
```
...
for (i, p) in enumerate(conf)
    ...
    haskey(p, "abstract") && print("[Abstract](" * p["abstract"] * "){.btn .btn-outline-primary .mx-1}")
    haskey(p, "poster") && print("[Poster](" * p["poster"] * "){.btn .btn-outline-info}")
    haskey(p, "slides") && print("[Slides](" * p["slides"] * "){.btn .btn-outline-warning}")
    println("")
    println("")
end
```